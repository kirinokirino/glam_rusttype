...
type DrawLineFn = unsafe fn(&mut Rasterizer, Vec2, Vec2);
...
impl Rasterizer {
    ...
    ///
    /// ```
    /// use glam_rusttype::rasterizer::Rasterizer;
    ...
    fn draw_line_scalar(&mut self, p0: Vec2, p1: Vec2) {
    ...
    fn tesselate_cubic(&mut self, p0: Vec2, p1: Vec2, p2: Vec2, p3: Vec2, n: u8) {
        ...
        let longlen = p0.distance(p1) + p1.distance(p2) + p2.distance(p3);
        let shortlen = p0.distance(p3);
...
unsafe fn draw_line_avx2(rast: &mut Rasterizer, p0: Vec2, p1: Vec2) {
    rast.draw_line_scalar(p0, p1)
}

#[cfg(all(feature = "std", any(target_arch = "x86", target_arch = "x86_64")))]
#[target_feature(enable = "sse4.2")]
unsafe fn draw_line_sse4_2(rast: &mut Rasterizer, p0: Vec2, p1: Vec2) {
    rast.draw_line_scalar(p0, p1)
}
