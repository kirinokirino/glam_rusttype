...
use crate::{Glyph, GlyphIter, IntoGlyphId, LayoutIter, Scale, VMetrics};
...
///
/// ```
/// # use glam_rusttype::font::RusttypeFont as Font;
/// # fn example() -> Option<()> {
...
impl RusttypeFont<'_> {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Option<RusttypeFont<'_>> {
    ...
    ///
    /// Returns `None` for invalid data.
    pub fn try_from_bytes_and_index(bytes: &[u8], index: u32) -> Option<RusttypeFont<'_>> {
        let inner = Arc::new(owned_ttf_parser::Face::parse(bytes, index).ok()?);
        Some(RusttypeFont::Ref(inner))
    }

    /// Creates a Font from owned font data.
    ///
    /// Returns `None` for invalid data.
    pub fn try_from_vec(data: Vec<u8>) -> Option<RusttypeFont<'static>> {
    ...
    ///
    /// Returns `None` for invalid data.
    pub fn try_from_vec_and_index(data: Vec<u8>, index: u32) -> Option<RusttypeFont<'static>> {
        let inner = Arc::new(owned_ttf_parser::OwnedFace::from_vec(data, index).ok()?);
        Some(RusttypeFont::Owned(inner))
    }
}
